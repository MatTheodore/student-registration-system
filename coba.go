...
//PERLU MENAMBAHKAN OPSI NIM PADA ADMIN
...
func registerMahasiswa() {
	...
	if jurusan == "kembali" {
		...
		if findJurusanifExist(jurusan) {
			...
			fmt.Print(dataJurusan.data[dataJurusan.jumlahJurusan].namaJurusan)
		}
	}
	if findJurusanifExist(jurusan) {
		fmt.Println("NIM: ", dataJurusan.data[dataMahasiswa.counterRegister].NIM)
	}
	if findJurusanifExist(jurusan) {
		...
		loginAsMahasiswa()
		// if searchUsernameifExist(username) == username {
...
func pengumumanKelulusan() {
	...
	fmt.Println(namaJurusanLulus == "")
	fmt.Println(mahasiswaLulusAtauTidak.nama)
	fmt.Println(mahasiswaLulusAtauTidak.nilai)
...
func findNIMfromJurusan(nim string) int {
	var i int
	for i < dataMahasiswa.jumlahMahasiswa && (dataJurusan.data[i].NIM != "") {
		if dataJurusan.data[i].NIM == nim {
			return i
		}
		i++
	}
